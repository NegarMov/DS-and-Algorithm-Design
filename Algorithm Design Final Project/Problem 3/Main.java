...
public class Main {
    ...
    static int x; // the limit of the total score
    static int score = 0; // the score gained so far
    static int count = 0; // number of total answers
    static int[] s; // the score of each square
     ...
     */
    static void displayAnswer(int score) {
    ...
    public static void main(String[] args) {
        ...
        n = scn.nextInt();
        System.out.print("Minimum score(x): ");
        x = scn.nextInt();
        ...
        for (int i=0; i<n; i++) {
            ...
            i = backtrack(i);
            for (int k=0; k<i; k++) {
                if (score + s[p[i]] < x - (n - i - 1) * (n * n) || isThreatened(i, k)) {
                    p[i]++;
                    k = -1;
                }
                i = backtrack(i);
            ...
            if (i == n - 1) {
                ...
                if (valid && score + s[p[i]] >= x) {
                    count++;
                    displayAnswer(score + s[p[i]]);
                }
